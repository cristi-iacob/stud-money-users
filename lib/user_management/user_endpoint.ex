defmodule Api.UserEndpoint do
  ...
  get "/",
    ...
    Publisher.publish(
      ...
      %{
        :type => "USER_FIND_ALL",
        :details => "All users from were requested."
      }
    )
    conn
    |> put_status(200)
    ...
    case User.get(id) do
      {:ok, user} ->
        Publisher.publish(
          ...
          %{
            :type => "USER_FIND_ID",
            :details => "User with id #{user.id} was searched."
          }
        )
  ...
  post "/register",
    ...
    {:ok, service} = Api.Service.Auth.start_link
    {email, password, firstname, lastname, role, location} = {
      Map.get(conn.params, "email", nil),
    ...
    IO.puts("Registration request: #{email}, #{firstname}, #{lastname}, #{role}, #{location}")
    cond do
      is_nil(email) ->
